...
ניקוד 1-100: כמה הנושא חשוב/מבוקש.
חשוב: עליך לאזן את כיסוי הקטגוריות באתר. תן עדיפות גבוהה לקטגוריות עם פחות מאמרים.`;
...
- בלי מקף ארוך (—) אף פעם. השתמש במקף רגיל (-).
- בלי מילים פלצניות ובלי סימנים מזהי AI ("בנוסף", "יתרה מכך", "לסיכום" וכו').
- אנלוגיות מהחיים, דוגמאות מהשטח הישראלי, מחירים בשקלים.
- כל מושג טכני שמוזכר חייב הסבר קצר בסוגריים בפעם הראשונה.
- כותרת חייבת לכלול keyword עיקרי.
- meta description 120-160 תווים, מסקרן + keyword.
- intro: 2-4 פסקאות שמסבירות מה במאמר.
- 4-12 sections עם heading ופסקאות/רשימות.
- callouts (tip/warning/saving) במקומות רלוונטיים.
- FAQs: 3-6 שאלות נפוצות אמיתיות.
...
- ויזואליזציה: לפחות 2-3 sections חייבים להכיל inlineImagePrompt (תיאור באנגלית של תמונה ממחישה - כלי, חלק, טכניקה, דיאגרמה, פוזיציה) + inlineImageAlt בעברית. כל מושג, כלי או חלק שמוזכר חשוב להמחיש. סמן asInfographic: true כשהתמונה מסכמת מידע (טבלת השוואה, דיאגרמה).
- כשרלוונטי, הוסף specTable (טבלת מפרט/השוואה), checklist (צ'קליסט מעשי שניתן להדפיס), sources (2-5 מקורות חיצוניים סמכותיים עם URL מלא).
