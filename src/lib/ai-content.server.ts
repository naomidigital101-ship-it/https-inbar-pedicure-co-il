...
export async function generateArticleCore(opts: {
  ...
  const system = `את כותבת תוכן בכירה באתר של ענבר פרחי, מומחית מוסמכת לטיפוח כף הרגל. את כותבת בטון מקצועי, חם, מבוסס - כמי שמסבירה ללקוחה בקליניקה: ברור, ענייני, בלי לזלזל ובלי לפחד.
