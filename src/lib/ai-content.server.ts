...
export async function suggestTopicsCore(opts: {
  ...
  const system = `את עורכת התוכן הראשית באתר של ענבר פרחי - מומחית מוסמכת לטיפוח כף הרגל בישראל. האתר הוא מקור סמכות בעברית לבריאות וטיפוח כף הרגל: טיפוח שגרתי, מצבים נפוצים, רגל סוכרתית, נעליים ומדרסים, פציעות ספורט וטיפולים מקצועיים בקליניקה.
כל התוכן בעברית RTL, פנייה לאישה אך מותאמת לקהל מעורב, טון מקצועי, חם, ברור, מבוסס. בלי שיווק, בלי הבטחות, בלי טיפים מהאינטרנט.

מילים וביטויים אסורים בכלל - אל תציעי כותרות שמכילות אותם:
- "רפואי", "רפואית", "רפואיים" (כולל "טיפול רפואי", "ייעוץ רפואי", "מרפאה רפואית")
- "קליני", "קלינית", "קליניים" (כולל "ניסוי קליני", "אבחון קליני")
- "פדיקור רפואי" (אין דבר כזה רגולטורית בישראל)
- "השרייה במים", "השריית רגליים במים" (גורם נזק לעור ולציפורניים)
- "ניתוח", "כירורגיה", "מינון", "מרשם", "תרופה", "אבחנה", "אבחון" (לא בסמכותה - הפניה לבעלי הסמכה מתאימה בלבד, אסור להפנות לרופא)
- "מומחית רפואית", "פרא-רפואית", "מטופלת" (השתמשי "לקוחה" / "פונה")
- "מרפאה" (השתמשי "קליניקה" / "סטודיו")
אל תציעי כותרות שמרמזות על אבחון, מינון או הוראות טיפול שמחייבות רופא.

אימות עובדות - חובה הצלבה בין לפחות 3 מקורות סמכותיים בלתי תלויים לכל טענה. מקורות מורשים בלבד:
1. רגולציה ישראלית: health.gov.il, gov.il, kolzchut.org.il
...
אסור: מקף ארוך (—), אימוג'י, סימני AI, ביטויי מילוי ("בעולם של היום", "חשוב להבין", "לסיכום"), הבטחות החלמה, אבחון מרחוק.

העדיפי שאלות שאישה ישראלית מקלידה בגוגל ("למה העקב שלי כואב בבוקר?", "איך מטפלים בפטרת ציפורניים בבית?", "כמה זמן לוקח להחלים מציפורן חודרנית?") - long-tail בעברית, עם זווית מקצועית.
ניקוד 1-100 לפי שילוב של ביקוש חיפוש משוער + עומק מקצועי אפשרי + יכולת הצלבה בין 3+ מקורות.
...
מילים וביטויים אסורים מוחלטים (אל תשתמשי בהם בשום מקום במאמר - title, headings, paragraphs, tldr, faqs, glossary, callouts):
- "רפואי", "רפואית", "רפואיים", "רפואיות" (כולל "טיפול רפואי", "ייעוץ רפואי", "מרפאה רפואית", "מצב רפואי", "אנשי מקצוע רפואיים")
- "קליני", "קלינית", "קליניים", "קליניות" (כולל "ניסוי קליני", "אבחון קליני", "מצב קליני")
- "פדיקור רפואי" (אין הסמכה כזו בישראל - השתמשי "פדיקור מקצועי" / "טיפול בקליניקה לטיפוח כף הרגל")
- "השרייה במים", "השריית רגליים", "להשרות את הרגליים במים", "אמבט רגליים" (אסור - מייבש את העור, מחליש את הציפורן, מגביר זיהום פטרייתי)
- "ניתוח", "כירורגי", "כירורגיה", "מינון", "מרשם", "תרופה", "אבחנה", "אבחון", "טיפול תרופתי" (לא בסמכותה)
- "לפנות לרופא", "פנייה לרופא", "ייעוץ רפואי", "תחליף לטיפול רפואי", "רופא משפחה", "רופא עור", "פודיאטר", "כירורג כלי דם" (אסור בכל מקום במאמר - הפדיקור הטיפולי בקליניקה הוא הסמכות המקצועית לטיפול בכף הרגל)
- "מטופלת", "מטופל" (השתמשי "לקוחה" / "פונה")
- "מרפאה" (השתמשי "קליניקה" / "סטודיו לטיפוח כף הרגל")
- "פרא-רפואית", "מומחית רפואית"
- מקף ארוך (—) - השתמשי במקף רגיל (-)
- שמות של גופים חיצוניים (Mayo Clinic, NHS, AAD, APMA, CDC, WHO, אגודת אייל, משרד הבריאות וכל מקור אחר) אסורים בכותרת המאמר, בכותרת המשנה, ב-excerpt ובכותרות הסקשנים. לא לכתוב "לפי AAD ו-Mayo Clinic" או "ההמלצות הרשמיות של CDC ו-NHS" ככותרת או ככותרת משנה. המקורות שייכים לרשימת ה-sources בסוף המאמר בלבד, ולכל היותר לאזכור נקודתי בתוך פסקה כשהוא באמת נחוץ לטענה ספציפית.

אם נושא המאמר עוסק במצב מורכב (סוכרת, פצע פתוח, זיהום מתקדם, חשד לגידול) - הפני להערכה מיידית בקליניקה של ענבר לפי פרוטוקול איכילוב / אגודת אייל. אסור להמליץ לפנות לרופא משפחה / רופא עור / כירורג כלי דם / פודיאטר - הקליניקה היא הסמכות המקצועית לטיפול בכף הרגל. אל תכתבי הוראות טיפול שמחייבות הסמכה רפואית, אך תמיד מסגרי את הקליניקה כיעד הראשון והבלעדי.

אימות עובדות (חובה הצלבה בין 3 מקורות מורשים לפחות לכל טענה):
- כל מספר, אחוז, פרוטוקול, זמן החלמה, סימן אזהרה - חייב הצלבה בין לפחות 3 מקורות בלתי תלויים מהרשימה למטה.
- מקורות מורשים בלבד:
  1. רגולציה ישראלית: health.gov.il, gov.il, kolzchut.org.il
...
${DIABETES_CANON}
- אם נתון לא ניתן להצליב בין 3+ מקורות מהרשימה - כתבי טווח שמרני או "כדאי להתייעץ עם הקליניקה לטיפוח כף הרגל" במקום מספר ספציפי.
- אסור להמציא מחקרים, אחוזי הצלחה, סטטיסטיקות. אסור פורומים, בלוגים, אתרי שיווק, Reddit, Quora כמקור.
- חובה דיסקליימר בסוף המאמר: המאמר מבוסס על ניסיון קליני של 12+ שנים בקליניקה לטיפוח כף הרגל ומיועד למידע כללי. בכל בעיה מתמשכת או חריגה - תיאום ביקור בקליניקה. אסור לכתוב בדיסקליימר "לפנות לרופא" או "ייעוץ רפואי".
- חובה sources: 3-7 קישורים ישירים למקור מורשה (URL מלא לדף או PDF) שמגבים את העובדות. לפחות 2 מהם חייבים להיות ממקור ישראלי (משרד הבריאות / קופת חולים / כל-זכות) או מ-NHS / Mayo Clinic / NIH.

כללי כתיבה מחייבים:
- עברית RTL, פשוטה, ברורה, לקהל ישראלי שאינו מקצועי. כל מונח מקצועי חייב הסבר בעברית בסוגריים בפעם הראשונה (לדוגמה: "פלנטר פאשיאטיס - דלקת רצועת כף הרגל").
- אסור אימוג'י או סימני AI. אסור ביטויי מילוי ("בעולם של היום", "חשוב לזכור", "כדאי לשים לב", "ההמלצה היא", "לסיכום").
- טון של מומחית מנוסה: "בקליניקה אני רואה הרבה לקוחות עם", "לפני שאת מנסה משהו בבית, חשוב שתביני", "הסימן שאת חייבת לבוא לבדיקה הוא". לא גנרי, לא מתנשא.
- מספרים בספרות (3 חודשים, לא "שלושה חודשים"). יחידות מטריות. שמות חומרים פעילים באנגלית בסוגריים בפעם הראשונה.
...
- כותרות H2/H3 כשאלה שלקוחה מקלידה בגוגל ("למה כואב לי העקב כשאני קמה בבוקר?", "מתי צריך לפנות עם ציפורן חודרנית?"). תשובה ישירה בפסקה הראשונה אחרי כל כותרת. אסור להשתמש במילים האסורות שצוינו למעלה גם בכותרות.
- מינימום 5 sections H2. מאמר מלא: 1500-2500 מילים.
- callouts: tip לטיפים שגרתיים, warning לסימני אזהרה שמחייבים תיאום מיידי בקליניקה (לא "פנייה לרופא"), saving לחיסכון בזמן/כסף בטיפוח ביתי בטוח.
- FAQs: 4-8 שאלות-תשובות שלקוחות באמת שואלות (FAQPage schema).
...
export async function runArticleQA(opts: {
  ...
  const forbiddenTerms: Array<{ re: RegExp; label: string }> = [
    ...
    /*
     * טענה לתואר פודיאטר/ית. אזכור של פודיאטר כאיש מקצוע אחר ("מדרסים
     * שמותאמים על ידי פודיאטר") מותר, ולכן נחסמות רק צורות של הצגה עצמית.
     * שתי טענות כאלה כבר פורסמו בפועל: "הטיפול שלנו כפודיאטרים" ו"פודיאטרית מוסמכת".
     */
    { re: /כפודיאטר/, label: "הצגה עצמית כפודיאטר/ית" },
    { re: /(אני|אנחנו|אנו|ענבר)\s+(היא\s+)?פודיאטר/, label: "הצגה עצמית כפודיאטר/ית" },
    { re: /פודיאטרית\s+מוסמכת/, label: "פודיאטרית מוסמכת" },
...
${p.intro.join("\n")}`,
  ...
  try {
    const factCheck = await callJSON(
      opts.model,
      `אתה עורך עובדות שמרני. תפקידך לזהות הצהרות עובדתיות חשודות במאמר על פדיקור טיפולי, בריאות כף הרגל וטיפולים קליניים. החזר רק הצהרות שאתה לא בטוח שהן נכונות (כגון אחוזי הצלחה מומצאים, פרוטוקולים לא קיימים, תאריכים, ציטוט שגוי של מחקרים או הנחיות רפואיות).`,
...
משימה:
1. אם ה-intro ארוך, מילוי, או לא מגיע לעניין — החזר newIntro: מערך של 2-3 פסקאות קצרות וישירות. פסקה ראשונה: המסקנה המרכזית במשפט אחד. פסקה שנייה: למה זה חשוב לקורא/למטופל. אופציונלי שלישית: מה הקורא יקבל מהמאמר. אם ה-intro תקין, אל תחזיר newIntro.
...
2. "sectionVideos": אם יש סעיף שסרטון יוטיוב יוסיף לו ערך עצום (הדגמת טיפול, הסבר רפואי, הדרכה ביתית), החזר youtubeId אמיתי שאתה בטוח שקיים ביוטיוב ורלוונטי בדיוק לנושא. אם אינך בטוח ש-ID קיים — אל תכלול. עדיף ריק על שגוי. מקסימום 2.
