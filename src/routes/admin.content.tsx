...
function TopicsTab({ onArticleCreated }: { onArticleCreated?: () => void }) {
  ...
  return (
    <div>
      ...
      <div className="mb-6 border-2 border-[#5fa898] bg-gradient-to-br from-[#1a0a06] to-[#fdfbf7] p-6 shadow-[0_0_40px_rgba(230,48,0,0.15)]">
        <div className="mb-3 flex items-center justify-between">
          <div className="text-[11px] font-black uppercase tracking-[0.3em] text-[#5fa898]">
            🚀 [ AUTOPILOT — הפעלה אחת, כל התהליך ]
          </div>
        </div>
        <p className="mb-3 text-base font-bold leading-relaxed text-[#1d3a35]">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3 text-xs text-[#2d4a44]">
          <label className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-16 border border-[#c9b8a3] bg-[#fdfbf7] px-2 py-1 text-center font-bold text-[#1d3a35]"
            />
            נושאים חדשים
          </label>
        ...
        <button
          ...
          className="w-full bg-[#5fa898] px-8 py-4 text-base font-black uppercase tracking-widest text-[#fdfbf7] hover:bg-[#ff3a00] disabled:opacity-60"
        >
          {autopilot.isPending ? "⏳ מריץ אוטופיילוט... אל תסגור את הדף" : "🚀 הפעל אוטופיילוט מלא"}
        </button>
        {apProgress && (
          <div className="mt-4 border border-[#5fa898] bg-[#fdfbf7] p-4">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-[10px] font-black uppercase tracking-widest text-[#5fa898]">
            ...
            <div className="mb-2 grid grid-cols-5 gap-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <div
                  key={s}
                  className={`h-1 ${s < apProgress.step ? "bg-[#5fa898]" : s === apProgress.step ? "bg-[#ff3a00]" : "bg-[#b8dcd4]"}`}
                />
              ))}
            </div>
            <div className="text-xs text-[#2d4a44]">{apProgress.detail}</div>
            {apProgress.log.length > 0 && (
              <details className="mt-2">
                <summary className="cursor-pointer text-[10px] font-bold uppercase tracking-wider text-[#5a4f48] hover:text-[#1d3a35]">
                  יומן ריצה ({apProgress.log.length})
                </summary>
                <div className="mt-2 max-h-40 overflow-y-auto border border-[#b8dcd4] bg-[#000] p-2 font-mono text-[10px] text-[#5a4f48]">
      ...
      <div className="mb-4 border border-[#b8dcd4] bg-[#fdfbf7] p-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="bg-[#5fa898] px-4 py-2 text-xs font-black uppercase tracking-wider text-[#fdfbf7] hover:bg-[#ff3a00] disabled:opacity-60"
        >
      ...
      <div className="overflow-x-auto border border-[#b8dcd4]">
        <table className="w-full text-right">
          ...
          <tbody>
            {(q.data ?? []).map((t) => (
              <tr key={t.id} className="border-t border-[#b8dcd4] text-sm text-[#2d4a44]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {t.status === "pending" && (
                      <>
                        ...
                        <button
                          ...
                          className="border border-[#7a1a1a] px-2 py-1 text-[10px] font-bold uppercase text-[#5fa898]"
                        >
                          מחק
                        </button>
                      </>
                    ...
                    {(t.status === "approved" || t.status === "pending") && (
                      <button
                        ...
                        className="bg-[#5fa898] px-2 py-1 text-[10px] font-bold uppercase text-[#fdfbf7] disabled:opacity-60"
                      >
...
function DetailDialog({
  ...
  return (
    ...
    >
      <div
        className="max-h-[85vh] w-full max-w-3xl overflow-y-auto border-2 border-[#5fa898] bg-[#fdfbf7] p-6"
        onClick={(e) => e.stopPropagation()}
        dir="rtl"
      >
        <div className="mb-4 flex items-start justify-between gap-4 border-b border-[#b8dcd4] pb-3">
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-[#5fa898]">
              {detail.kind === "fact" ? "[ אימות עובדות — פירוט מלא ]" : "[ דוח QA — פירוט מלא ]"}
            </div>
            <h3 className="mt-1 text-lg font-black text-[#1d3a35]">{detail.title}</h3>
          </div>
          <button
            onClick={onClose}
            className="border border-[#444] px-3 py-1 text-xs font-black uppercase text-[#2d4a44] hover:border-[#5fa898] hover:text-[#5fa898]"
            aria-label="סגור"
          >
            סגור ✕
          </button>
...
function FactDetail({ report }: { report: FactCheckReport }) {
  ...
  return (
    <div className="space-y-4 text-sm text-[#2d4a44]">
          ...
          {corrections.map((c, i) => (
            ...
            >
              <div className="mb-2 flex items-center justify-between gap-2">
                <span
                  className={`px-2 py-0.5 text-[10px] font-black uppercase ${
                    c.applied ? "bg-[#0a8a3a] text-[#fdfbf7]" : "bg-[#c98a00] text-[#fdfbf7]"
                  }`}
                >
              ...
              <div className="space-y-1.5 text-xs leading-relaxed">
                <div>
                  <span className="font-black text-[#5fa898]">לפני: </span>
                ...
                <div className="border-t border-[#b8dcd4] pt-1.5">
                  <span className="font-black text-[#2d4a44]">מה היה לא בסדר: </span>
        ...
        <ul className="space-y-2">
          {issues.map((it, i) => (
            ...
            >
              ...
              <p className="text-xs leading-relaxed text-[#2d4a44]">{it.message}</p>
            </li>
          ))}
        </ul>
      )}
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border border-[#b8dcd4] bg-[#e9f4f1] p-3 text-center">
      <div className="text-[10px] font-bold uppercase tracking-wider text-[#5a4f48]">{label}</div>
      <div className="mt-1 text-2xl font-black text-[#1d3a35]">{value}</div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className="border border-[#c9b8a3] px-2 py-1 text-[10px] font-bold uppercase text-[#2d4a44]"
        >
          🔄 רענן
        </button>
        <button
          onClick={submitAllMissing}
          className="bg-[#5fa898] px-3 py-1 text-[10px] font-black uppercase text-[#fdfbf7]"
        >
      ...
      {submitResult && (
        <div className="border border-[#5fa898] bg-[#faf3eb] p-4 text-xs text-[#2d4a44]">
          <div className="mb-2 flex items-center justify-between">
            <b className="text-[#5fa898]">תוצאת הגשה</b>
            <button
              onClick={() => setSubmitResult(null)}
              className="text-[10px] text-[#7a6e65] hover:text-[#2d4a44]"
            >
              סגור ✕
            </button>
          </div>
          <ul className="space-y-1 text-[11px]">
            ...
            <li>
              ...
              <b className={submitResult.indexNow.ok ? "text-[#0a8a3a]" : "text-[#5fa898]"}>
            ...
            <li>
              ...
              <b className={submitResult.sitemapResubmitted ? "text-[#0a8a3a]" : "text-[#5fa898]"}>
          ...
          {submitResult.inspectionLinks.length > 0 && (
            <details className="mt-3">
              <summary className="cursor-pointer text-[11px] font-bold text-[#2d4a44]">
      ...
      <div className="overflow-x-auto border border-[#b8dcd4]">
        <table className="w-full text-right text-sm text-[#2d4a44]">
          <thead className="bg-[#e9f4f1] text-[10px] font-black uppercase tracking-wider text-[#5a4f48]">
            <tr>
              <th className="px-3 py-2">כותרת</th>
    ...
    <div className="max-w-2xl space-y-4">
      <Field label="מאמרים בשבוע">
        <input
          ...
          className="w-full border border-[#b8dcd4] bg-[#fdfbf7] px-3 py-2 text-[#1d3a35]"
        />
      </Field>
      ...
      <Field label="פרסום אוטומטי (ללא אישור ידני)">
        <label className="flex items-center gap-2 text-sm text-[#2d4a44]">
          <input
            type="checkbox"
      ...
      <button
        ...
        className="bg-[#5fa898] px-6 py-3 text-xs font-black uppercase tracking-wider text-[#fdfbf7] disabled:opacity-60"
      >
...
function AdminSection({
  ...
  return (
    <section id={id} className="space-y-4 border border-[#b8dcd4] bg-[#e9f4f1] p-4 md:p-6">
      <div className="flex items-center justify-between gap-3 border-b border-[#b8dcd4] pb-3">
        <h2 className="text-base font-black text-[#1d3a35]">{title}</h2>
      </div>
      {children}
    </section>
  );
...
function AutopilotHistoryTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[#b8dcd4] bg-[#fdfbf7] p-3 text-sm text-[#2d4a44]">
        <div>
        ...
        <button
          ...
          className="border border-[#5fa898] bg-[#5fa898] px-4 py-2 text-xs font-black uppercase tracking-wider text-white transition-colors hover:bg-[#c52800] disabled:opacity-50"
        >
                    ...
                    <ul className="space-y-2">
                      {results.map((r, i) => (
                        <li key={i} className="flex flex-wrap items-center gap-2 rounded bg-[#e9f4f1] p-2 text-xs">
                          ...
                          <span className="font-semibold text-[#1d3a35]">{r.topicTitle}</span>
                          {r.slug && (
                            <a
                              href={`/article/${r.slug}`}
                              target="_blank"
